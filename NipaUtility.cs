...
namespace NipaFriends
{
    public static class NipaUtility
    {
        ...
        /// [note] : -
        ///</summary> 
        public static bool IsFaceToTaget(Transform _observer, Vector3 _tgtPos, float _limitAngle, bool _ignoreHeight = false)
        {
            if (_ignoreHeight)
            {
            ...
            return Quaternion.Angle(_observer.rotation, Quaternion.LookRotation(_tgtPos - _observer.position)) < _limitAngle;
        }
        ...
        public static Vector2 GetDirectionAngleToOriginalDir(Vector2 _original, float _ang)
        {
            ...
            return new Vector2(_original.x * Mathf.Cos(_ang) - _original.y * Mathf.Sin(_ang), _original.x * Mathf.Sin(_ang) + _original.y * Mathf.Cos(_ang));
        }
        ...
        /// [note] : -
        ///</summary> 
        public static Vector3 GetPositionRayHitPlane(Vector3 planePos, Vector3 planeNormalDir, Vector3 rayStartPos, Vector3 rayDir)
        {
            return rayStartPos + ((Vector3.Dot(planeNormalDir, planePos) - Vector3.Dot(planeNormalDir, rayStartPos)) / (Vector3.Dot(planeNormalDir, rayDir))) * rayDir;
        }
        ...
        ///</summary> 
        public static bool GetPositionOffToTarget(Vector3 _posFrom, Vector3 _posGoal, Vector3 _posCenter, float _dist, out Vector3 _result)
        {
            _result = _posFrom;
        ...
        {
            return new Vector2(target.x * rotNormarized.x - target.y * rotNormarized.y, target.x * rotNormarized.y + target.y * rotNormarized.x);
        }
        ...
        /// [note] : -
        ///</summary> 
        public static bool GetCrossPoint(Vector2 a1, Vector2 a2, Vector2 b1, Vector2 b2, out Vector2 result, out float ratio)
        {
            ratio = 0f;
            var v = b1 - a1;
            var va = a2 - a1;
        ...
        /// [note] : -
        ///</summary> 
        public static Vector3[] GetFormation_Line(Vector3[] _positions, Vector3 _desti, float _interval, bool _sort = false)
        {
        ...
        ///[note] : 同名ファイルには書き足す
        ///</summary> 
        public static bool WriteTextFile(string _dirPath, string _fileName, string _content, StringDateType _minTimeType, string _extension = ".txt")
        {
            ...
            var path = Path.Combine(_dirPath, _fileName);
